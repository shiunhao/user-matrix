/* ============================================================================
 * AVer TR615 — Camera & Paint/Look WEB UI 原型  (V4.2)
 * ----------------------------------------------------------------------------
    ...
    <div id="aver-paint-look-root" style={{ position: "relative", background: T.page, width: "100%", height: "100vh", fontFamily: fUI, color: T.text, display: "flex", overflow: "hidden" }}>
      ...
      <div id="aver-main-stage" style={{ position: "relative", flex: 1, padding: "16px 24px", minWidth: 0, background: T.page, overflow: "hidden", height: "100vh", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
        {activeMenu === "paint" ? (
          ...
          <div id="aver-live-view-wrapper" key="live" className="aver-fade" style={{ display: "flex", flexDirection: "column", gap: 12, width: "100%", maxWidth: "1350px", margin: "0 auto", height: "100%", minHeight: 0 }}>
            {(() => {
                  ...
                  <div style={{ background: T.panel, border: `1px solid ${T.line}`, borderRadius: 10, flex: "0 0 auto", display: "flex", flexDirection: "column", overflow: "hidden" }}>
                    ...
                    <div style={{ display: "flex", borderBottom: `1px solid ${T.line}` }}>
                      {[["control", "Camera Control"], ["preset", "Preset"]].map(([id, lb]) => (
                        <button key={id} onClick={() => updLive("tab", id)}
                          style={{ flex: "0 0 200px", padding: "11px 0", fontSize: 14, fontWeight: 600, cursor: "pointer", border: "none", background: live.tab === id ? T.blue : "transparent", color: live.tab === id ? "#fff" : T.dim, fontFamily: fUI }}>
                          {lb}
                        </button>
                      ))}
                    </div>
                    ...
                    {live.tab === "control" ? (
                      /* ===== Camera Control ===== */
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 16, padding: 16, alignItems: "flex-start", minHeight: 280, boxSizing: "border-box", alignContent: "flex-start" }}>
                        {/* 方向盤 + Zoom */}
                        <div style={{ ...sec, display: "flex", gap: 14, alignItems: "center" }}>
                          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 42px)", gridTemplateRows: "repeat(3, 42px)", gap: 5 }}>
                            <span />
                            <button style={sqStyle(false)}>▲</button>
                            <span />
                            <button style={sqStyle(false)}>◀</button>
                            <button style={{ ...sqStyle(false), borderRadius: "50%", fontSize: 15 }}>⌂</button>
                            <button style={sqStyle(false)}>▶</button>
                            <span />
                            <button style={sqStyle(false)}>▼</button>
                            <span />
                        ...
                        <div style={{ ...sec, display: "flex", gap: 14 }}>
                          <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                            <button onClick={() => updLive("focusMode", "af")} style={sqStyle(live.focusMode === "af")}>AF</button>
                            <button onClick={() => updLive("focusMode", "mf")} style={sqStyle(live.focusMode === "mf")}>MF</button>
                            <button title="One-Push AF" style={sqStyle(false)}>◎</button>
                          </div>
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                            <span style={{ fontSize: 12, color: T.faint }}>Focus</span>
                            <button disabled={live.focusMode !== "mf"} style={{ ...sqStyle(false), opacity: live.focusMode !== "mf" ? 0.4 : 1, cursor: live.focusMode !== "mf" ? "not-allowed" : "pointer" }}>＋</button>
                            <button disabled={live.focusMode !== "mf"} style={{ ...sqStyle(false), opacity: live.focusMode !== "mf" ? 0.4 : 1, cursor: live.focusMode !== "mf" ? "not-allowed" : "pointer" }}>－</button>
                          </div>
                          <div style={{ display: "flex", flexDirection: "column", gap: 12, justifyContent: "flex-start", minWidth: 130 }}>
                            <div>
                              ...
                              <select value={live.focusNear} onChange={(e) => updLive("focusNear", e.target.value)} style={{ width: "100%", padding: "7px 9px", fontSize: 13, borderRadius: 6, border: `1px solid ${T.line2}`, background: T.panel2, color: T.text, fontFamily: fUI, cursor: "pointer" }}>
                                {["1cm", "11cm", "30cm", "50cm", "80cm", "1m", "1.5m", "2m", "3m", "5m", "∞"].map((v) => <option key={v} value={v}>{v}</option>)}
                              </select>
                            </div>
                            <div>
                              <div style={secTitle}>AF Mode</div>
                              <select value={live.afMode} onChange={(e) => updLive("afMode", e.target.value)} style={{ width: "100%", padding: "7px 9px", fontSize: 13, borderRadius: 6, border: `1px solid ${T.line2}`, background: T.panel2, color: T.text, fontFamily: fUI, cursor: "pointer" }}>
                        ...
                        <div style={{ ...sec, minWidth: 220, flex: "1 1 220px", display: "flex", flexDirection: "column", gap: 12 }}>
                          <ExpSlider label="Pan Speed" leftLabel="1" rightLabel="24" valueText={"" + live.panSpeed} min={1} max={24} val={live.panSpeed} onChange={(v) => updLive("panSpeed", v)} />
                          <ExpSlider label="Tilt Speed" leftLabel="1" rightLabel="24" valueText={"" + live.tiltSpeed} min={1} max={24} val={live.tiltSpeed} onChange={(v) => updLive("tiltSpeed", v)} />
                          <div>
                            <div style={secTitle}>Zoom Speed</div>
                            <div style={{ display: "flex", gap: 28 }}>
                              <CamRadio label="High" checked={live.zoomSpeed === "high"} onChange={() => updLive("zoomSpeed", "high")} />
                              <CamRadio label="Low" checked={live.zoomSpeed === "low"} onChange={() => updLive("zoomSpeed", "low")} />
                            </div>
                          </div>
                        </div>
                        ...
                        <div style={{ ...sec, minWidth: 220, flex: "1 1 220px", display: "flex", flexDirection: "column", gap: 12 }}>
                          <div>
                            ...
                            <div style={{ display: "flex", gap: 28 }}>
                              <CamRadio label="On" checked={live.digitalZoom} onChange={() => updLive("digitalZoom", true)} />
                              <CamRadio label="Off" checked={!live.digitalZoom} onChange={() => updLive("digitalZoom", false)} />
                            </div>
                          </div>
                          <ExpSlider label="Digital Zoom Limit" leftLabel="x2" rightLabel="x12" valueText={"x" + live.digitalZoomLimit} min={2} max={12} val={live.digitalZoomLimit} onChange={(v) => updLive("digitalZoomLimit", v)} disabled={!live.digitalZoom} />
                          <CamCheck label="Relative Zoom Ratio" checked={live.relativeZoom} onChange={(v) => updLive("relativeZoom", v)} />
                          <CamCheck label="Preset Affects PTZ & Focus Values Only" checked={live.presetAffects} onChange={(v) => updLive("presetAffects", v)} />
                        </div>
                      </div>
                    ) : (
                      /* ===== Preset(預設位置)===== */
                      <div style={{ padding: 16, minHeight: 280, boxSizing: "border-box" }}>
                          ...
                          {Array.from({ length: 9 }, (_, i) => i + 1).map((n) => (
                            <div key={n} style={{ ...sec, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                              ...
                              <div style={{ display: "flex", gap: 6 }}>
                                <button style={{ padding: "5px 12px", fontSize: 12, cursor: "pointer", borderRadius: 5, border: "none", background: T.blue, color: "#fff", fontFamily: fUI }}>呼叫</button>
                                <button style={{ padding: "5px 12px", fontSize: 12, cursor: "pointer", borderRadius: 5, border: `1px solid ${T.line2}`, background: T.panel2, color: T.dim, fontFamily: fUI }}>設定</button>
                              </div>
                            </div>
                          ))}
                        </div>
