...
if os.path.exists(walkthrough_path):
    ...
    new_wt_25 = """### 25. 移除色相環節點上的「橘色已調整」標記與相關說明字串
* **改了什麼**：
  - **移除橘點標記**：因為色相環上的 node 會隨著使用者改變數值而即時在圓盤上移動與改變背景色彩，已具備直觀的調整狀態回饋，故移除額外的橘色小圓點標記。
  - **還原節點樣式**：已調整過的 node 元件在非選中狀態下，其 border 與 boxShadow 維持與普通 node 元件一致，不再顯示橘色外框或橘色光暈，維持乾淨簡約的視覺質感。
  - **移除提示字串**：移除右側操作面板中的提示文字「`點節點調整;橘點 = 已調整。每軸僅影響該色相範圍,不動其他顏色。`」，避免冗餘資訊。"""

    if "### 25." in content:
        start_idx = content.find("### 25.")
        end_idx = content.find("\n---", start_idx)
        if start_idx != -1 and end_idx != -1:
            content = content[:start_idx] + new_wt_25 + "\n" + content[end_idx:]
    else:
        end_idx = content.find("---", content.find("### 24."))
        if end_idx != -1:
            content = content[:end_idx] + new_wt_25 + "\n\n" + content[end_idx:]
