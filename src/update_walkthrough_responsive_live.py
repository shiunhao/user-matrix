...
if os.path.exists(walkthrough_path):
    ...
    new_wt_29 = """### 29. 修正 Live View 與 Camera Settings 頁面的 Live 預覽畫面響應式尺寸
* **改了什麼**：
  - **引進高度自適應 min() 寬度限制**：將 `Live View` 和 `Camera Settings` 兩個頁面的 16:9 Live 模擬畫面容器最大寬度限制重構為：
    `maxWidth: "min(880px, calc((100vh - 420px) * 1.777))"`
  - **解決控制面板 Overflow**：依據總高度動態扣除控制面板與邊界所需的高度（約 `420px`）自適應縮小預覽畫面。在低高度螢幕上它會自動等比收縮預覽圖，確保下方所有 Exposure、Camera Control 或是 Preset 等控制面板的按鈕與滑桿 100% 能完整顯示，消除縱向溢出問題。
  - **水平居中優化**：當畫幅受限收窄時，自動藉由 `margin: "0 auto"` 保持畫面居中，視覺比例更為精緻美觀，且底部的控制面板維持 100% 寬度不變。"""

    if "### 29." in content:
        start_idx = content.find("### 29.")
        end_idx = content.find("\n---", start_idx)
        if start_idx != -1 and end_idx != -1:
            content = content[:start_idx] + new_wt_29 + "\n" + content[end_idx:]
    else:
        end_idx = content.find("---", content.find("### 28."))
        if end_idx != -1:
            content = content[:end_idx] + new_wt_29 + "\n\n" + content[end_idx:]
...
if os.path.exists(task_path):
  ...
  - [x] 依照使用者回饋，將「已調整 the 軸」標題文字以及 Chip 標籤（包括內部 Padding、色彩指示點、名稱、數值、✕按鈕）按比例整體放大。
