...
if os.path.exists(walkthrough_path):
    ...
    new_wt_33 = """### 33. 實作 Live 預覽畫面高度填滿且僅在左右產生黑邊
* **改了什麼**：
  - **預覽圖高度始終填滿**：將 `Live View` 與 `Camera Settings` 頁面內層 16:9 預覽畫面的寬高定義重構為 **`height: "100%", width: "auto"`**，移除 `width: "100%"` 與 `maxHeight` 的多重限制。
  - **僅在左右留下黑邊 (Pillarboxing)**：因為高度被強制設為 `100%`，Live 模擬畫面會百分之百完美填滿外層的黑色大容器（消除所有上下黑邊），而寬度則透過 `aspectRatio: "16 / 9"` 自動計算，並在寬度有多餘時於畫面左右兩側留下黑邊。這確保了圖片內容比例完全正確不變形，且底部的控制面板維持 100% 原始高度並與底邊精準保持 **`16px`** 的 padding 距離。"""

    if "### 33." in content:
        start_idx = content.find("### 33.")
        end_idx = content.find("\n---", start_idx)
        if start_idx != -1 and end_idx != -1:
            content = content[:start_idx] + new_wt_33 + "\n" + content[end_idx:]
    else:
        end_idx = content.find("---", content.find("### 32."))
        if end_idx != -1:
            content = content[:end_idx] + new_wt_33 + "\n\n" + content[end_idx:]
...
if os.path.exists(task_path):
  ...
  - [x] 對齊 `Live View` 與 `Camera Settings` 的底部 padding 與 `Paint / Look` 一致（統一 gap 爲 10 且預覽圖最大高度限制調整為 `100vh - 460px`），確保控制面板 100% 原始大小不變的狀態下吸收全部高度收縮，杜絕 any 溢出。
  ...
  - [x] 優化 Live 預覽畫面使其高度始終百分之百填滿（`height: "100%", width: "auto"`），消除所有上下留黑邊的情況，僅在寬度有多餘時於左右兩側自適應留下黑邊，且控制面板底邊 padding 精確固定為 **16px**。
