...
if os.path.exists(walkthrough_path):
    ...
    new_wt_30 = """### 30. 精緻對齊 Live View 與 Camera Settings 頁面的底邊留白 (Padding)
* **改了什麼**：
  - **底部 padding 完全對齊**：將 `Live View` 與 `Camera Settings` 的主容器間距（gap）由 `12` 微調為 **`10`**，與 `Paint / Look` 頁面保持完全一致。
  - **進一步縮小預覽畫面以保留底邊 padding**：為了在任何視窗高度下都保留完美的底邊 `16px` 留白（不出現溢出與滾動條），並保證底部的控制面板維持 100% 原始大小不變，將預覽畫面的最大寬度限制扣除值由 `420px` 提升至 **`460px`**：
    `maxWidth: "min(880px, calc((100vh - 460px) * 1.777))"`
  - **視覺效果**：現在三個主要頁面的最底邊留白與網格對齊感完全相同，且底部的控制面板按鈕與滑桿在小螢幕上均能完整、無裁切地呈現，消除所有縱向溢出問題。"""

    if "### 30." in content:
        start_idx = content.find("### 30.")
        end_idx = content.find("\n---", start_idx)
        if start_idx != -1 and end_idx != -1:
            content = content[:start_idx] + new_wt_30 + "\n" + content[end_idx:]
    else:
        end_idx = content.find("---", content.find("### 29."))
        if end_idx != -1:
            content = content[:end_idx] + new_wt_30 + "\n\n" + content[end_idx:]
