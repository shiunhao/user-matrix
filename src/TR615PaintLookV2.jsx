...
function applyMulti(R, G, B, axes) {
  ...
  // 使用溫和 of 餘弦/線性權重，使相鄰色域過渡自然 (半寬度約 30 度)
...
// 內部運算解析度 (16:9 HD 格式)。設定為 1280x720 (720p HD) 以維持極致清晰與銳利的細節。
 ...
 * 數值滑動輸入組件 (Slider)
 */
function Slider({ k, label, hint, min, max, val, onChange, neutral = 0, onStartDrag, onEndDrag }) {
  return (
...
export default function App() {
  ...
  const [multiStyle, setMultiStyle] = useState("wheel");
  ...
  useEffect(() => {
    ...
    if (useDrag) {
      if (!window.__tempDragCanvas) {
        window.__tempDragCanvas = document.createElement("canvas");
        window.__tempDragCanvas.width = 320;
        window.__tempDragCanvas.height = 180;
      }
      const tempCtx = window.__tempDragCanvas.getContext("2d");
      tempCtx.putImageData(out, 0, 0);
      
      ctx.clearRect(0, 0, SW, SH);
      ctx.drawImage(window.__tempDragCanvas, 0, 0, 320, 180, 0, 0, SW, SH);
    } else {
  ...
  const renderBlock = () => {
    ...
    if (block === "multi") {
      ...
      const angUI = {
        "R": 0, "R+": 22.5, "YL-": 45, "YL": 67.5, "YL+": 90,
        "G-": 112.5, "G": 135, "G+": 157.5, "CY": 180, "CY+": 202.5,
        "B-": 225, "B": 247.5, "B+": 270, "MG-": 292.5, "MG": 315, "MG+": 337.5
      };

      // 事先計算 16 個點的固定 X、Y 座標與色相角度 (取消物理位移，確保按鈕與軸線物理位置永遠固定不亂動)
      const pts = AXIS16.map((a) => {
        const ang = (angUI[a] - 90) * Math.PI / 180;
        const rFixed = 114;
        return {
          name: a,
          x: 145 + Math.cos(ang) * rFixed,
          y: 145 + Math.sin(ang) * rFixed,
          ang: angUI[a]
        };
      });

      // conic-gradient 的動態旋轉度數與飽和度係數 (在聚焦態下啟用)
      const focusAx = isFocused ? st.axes[selAxis] : { hue: 0, sat: 0 };
      const rotateDeg = isFocused ? (focusAx.hue / 99) * 45 : 0; // 最大轉動 +/- 45 度
      const satFactor = isFocused ? Math.max(0, 1 + (focusAx.sat / 99)) : 1; // 飽和度係數 0% 到 200%

      // 聚焦態下，色相環底圖只呈現該節點的色相，而非全色彩虹環
      const focusHue = isFocused ? (angUI[selAxis] + (focusAx.hue / 99) * 30 + 360) % 360 : 0;
      const conicBg = isFocused
        ? `conic-gradient(from 0deg, hsl(${focusHue} 85% 60%), hsl(${focusHue} 40% 45%), hsl(${focusHue} 85% 60%))`
        : "conic-gradient(from 0deg, hsl(0 85% 58%), hsl(60 85% 55%), hsl(120 70% 50%), hsl(180 75% 52%), hsl(240 85% 62%), hsl(300 85% 60%), hsl(360 85% 58%))";
      ...
      return (
        <div id="aver-control-params-multi">
            ...
            right={
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div style={{ display: "flex", background: "#101216", border: `1px solid ${T.line}`, borderRadius: 6, padding: 3, gap: 4, alignItems: "center" }}>
                  <button 
                    onClick={() => { setMultiStyle("wheel"); setIsFocused(false); }} 
                    style={{ 
                      padding: "4px 10px", fontSize: 13, cursor: "pointer", borderRadius: 4, border: "none", 
                      background: multiStyle === "wheel" ? T.blue : "transparent", 
                      color: multiStyle === "wheel" ? "#fff" : T.dim, fontFamily: fUI,
                      transition: "all .15s"
                    }}
                  >
                    雷達色環
                  ...
                  <button 
                    onClick={() => { setMultiStyle("eq"); setIsFocused(false); }} 
                    style={{ 
                      padding: "4px 10px", fontSize: 13, cursor: "pointer", borderRadius: 4, border: "none", 
                      background: multiStyle === "eq" ? T.blue : "transparent", 
                      color: multiStyle === "eq" ? "#fff" : T.dim, fontFamily: fUI,
                      transition: "all .15s"
                    }}
                  >
                    色彩等化器
          ...
          />
          <div style={{ opacity: st.multiOn ? 1 : 0.35, pointerEvents: st.multiOn ? "auto" : "none", display: "flex", flexDirection: "column", gap: 24, width: "100%", boxSizing: "border-box" }}>
            ...
            {multiStyle === "wheel" ? (
              <div style={{ display: "flex", gap: 28, flexWrap: "wrap", alignItems: "center", width: "100%" }}>
                ...
                <div style={{ position: "relative", width: 290, height: 290, flexShrink: 0 }}>
                  ...
                  <div style={{ 
                    ...
                    borderRadius: "50%", 
                    background: conicBg, 
                    filter: `blur(18px) saturate(${satFactor})`, 
                    opacity: 0.22,
                    transform: `rotate(${rotateDeg}deg)`,
                    transition: "transform 0.15s ease-out, filter 0.15s ease-out, background 0.25s ease-out"
                  }} />
                  <div style={{ 
                    position: "absolute", 
                    inset: 16, 
                  ...
                  <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 5 }}>
                    {/* 16 條放射狀均勻參考虛線 */}
                    {AXIS16.map((a) => {
                      const rAng = (angUI[a] - 90) * Math.PI / 180;
                      const rx = 145 + Math.cos(rAng) * 114;
                      const ry = 145 + Math.sin(rAng) * 114;
                      const lineHide = isFocused && a !== selAxis;
                      return (
                        <line 
                          key={a}
                          x1={145} 
                    ...
                    <polygon
                      ...
                      fill="rgba(30, 155, 240, 0.09)"
                      stroke="rgba(30, 155, 240, 0.55)"
                      strokeWidth="1.5"
                      style={{ 
                  ...
                  {/* 16 軸動態位置按鈕點 */}
                  {pts.map((p) => {
                    const axObj = st.axes[p.name];
                    const touched = axObj.hue !== 0 || axObj.sat !== 0;
                    const isSel = selAxis === p.name;
                    const [r, g, b] = hsv2rgb(p.ang, 0.85, 0.95);
                    
                    // 聚焦態隱藏邏輯：若為聚焦態且此點不是選中點，則淡出隱藏
                    ...
                    return (
                      <button 
                        ...
                        onClick={() => {
                          setSelAxis(p.name);
                          setIsFocused(true); // 點點進入聚焦態
                        }} 
                        style={{
                          position: "absolute", left: p.x, top: p.y, transform: "translate(-50%,-50%)",
                          width: isSel ? 32 : 24, height: isSel ? 32 : 24, borderRadius: "50%", cursor: hide ? "default" : "pointer",
                          background: `rgb(${r * 255},${g * 255},${b * 255})`,
                          border: isSel ? "2px solid #fff" : touched ? `2px solid ${T.amber}` : "1px solid rgba(0,0,0,0.55)",
                          boxShadow: isSel ? `0 0 16px hsl(${p.ang} 90% 60% / 0.85)` : touched ? `0 0 8px ${T.amber}66` : "none",
                  ...
                  <div style={{ 
                    ...
                    inset: 68, 
                    borderRadius: "50%", 
                    background: "radial-gradient(circle at 38% 30%, #181c21, #0e1114)", 
                    border: `1.5px solid ${isFocused ? T.blue : T.line2}`, 
                    ...
                    {isFocused ? (
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "100%", height: "100%", justifyContent: "center", padding: "10px", boxSizing: "border-box" }}>
                        <span style={{ fontSize: 13, letterSpacing: 1.5, color: T.blue, fontWeight: 700, fontFamily: fMono, marginBottom: 2 }}>FOCUS</span>
                        <span style={{ fontSize: 24, fontWeight: 800, color: "#fff", lineHeight: 1 }}>{selAxis}</span>
                        <span style={{ fontSize: 13, color: T.faint, fontFamily: fMono, margin: "2px 0" }}>{AXIS_HUE[selAxis]}°</span>
                        <div style={{ fontSize: 13.5, fontFamily: fMono, color: T.dim, display: "flex", gap: 6 }}>
                          <span>H{ax.hue >= 0 ? "+" + ax.hue : ax.hue}</span>
                          <span>S{ax.sat >= 0 ? "+" + ax.sat : ax.sat}</span>
                        </div>
                        <button 
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          style={{
                            ...
                            fontWeight: 600,
                            color: "#fff",
                            background: "rgba(30, 155, 240, 0.18)",
                ...
                <div style={{ flex: 1, minWidth: 220 }}>
                  <div style={{ fontSize: 14, color: T.dim, marginBottom: 12 }}>
                    ...
                    <span style={{ color: T.faint }}> · {isFocused ? "已聚焦！拖拉可使整個色相環即時旋轉變色" : "點色環節點切換或拖拉，亮起 = 已調整"}</span>
                  </div>
                  <Slider k="hue" label="Hue" hint="該區色相" min={-99} max={99} val={ax.hue} onChange={(v) => updAxis(selAxis, "hue", v)} onStartDrag={startDrag} onEndDrag={endDrag} />
                  <Slider k="sat" label="Saturation" hint="該區飽和度" min={-99} max={99} val={ax.sat} onChange={(v) => updAxis(selAxis, "sat", v)} onStartDrag={startDrag} onEndDrag={endDrag} />
              ...
              <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ fontSize: 14, color: T.dim, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <button 
                    onClick={() => upd("axes", DEF_AXES())}
                    style={{ background: "none", border: "none", color: T.blue, cursor: "pointer", fontSize: 14, fontFamily: fUI, padding: 0 }}
                  >
                    全部重設歸零
                  </button>
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ padding: "6px 8px 4px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: `1px solid ${T.line}` }}>
                          ...
                          {touched && (
                            <button 
                              onClick={(e) => { e.stopPropagation(); updAxis(a, "hue", 0); updAxis(a, "sat", 0); }}
                              style={{ background: "none", border: "none", cursor: "pointer", color: T.faint, fontSize: 13, padding: 0, lineHeight: 1 }}
                              title="此軸重設"
                            >
                              ✕
                            </button>
