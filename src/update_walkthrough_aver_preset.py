...
if os.path.exists(walkthrough_path):
    ...
    new_wt_24 = """### 24. 將原廠預設場景名稱從「Standard」更名為「AVer」
* **改了什麼**：
  - **預設場景更名**：將原廠最底層的預設場景（不可刪除）由「Standard」全面更名為 **「AVer」**。
  - **同步更新相關聯組件**：
    - 經典佈局（Classic）與劇院佈局（Cinema）下的原廠場景卡片標籤顯示為 **`AVer`**。
    - 當前載入場景狀態文字更名為 **`AVer (原廠預設)`**。
    - 載入原廠設定時的 Toast 提示訊息變更為 **`已載入 AVer (原廠預設)`**。"""

    if "### 24." in content:
        start_idx = content.find("### 24.")
        end_idx = content.find("\n---", start_idx)
        if start_idx != -1 and end_idx != -1:
            content = content[:start_idx] + new_wt_24 + "\n" + content[end_idx:]
    else:
        end_idx = content.find("---", content.find("### 23."))
        if end_idx != -1:
            content = content[:end_idx] + new_wt_24 + "\n\n" + content[end_idx:]
